...
describe('questionsSlice', () => {
    ...
    test('should remove all questions for the list', () => {
        expect(reducer(currentState, clearQuestions())).toEqual({
            questions: [],
        });
    });
    ...
    test('should add a new question to the list', () => {
        ...
        expect(currentState.questions[1].question).toEqual(
    ...
    test('should sort the questions by label', () => {
        ...
        expect(currentState.questions[0].question).toEqual(
            oldState.questions[2].question,
        );
        expect(currentState.questions[1].question).toEqual(
            oldState.questions[0].question,
        );
        expect(currentState.questions[2].question).toEqual(
            oldState.questions[1].question,
        );
    });

    test('should show the answer for a question', () => {
        expect(
            reducer(currentState, showAnswer(currentState.questions[0]))
                .questions[0].isVisible,
        ).toBeTruthy();
        expect(
            reducer(currentState, showAnswer(currentState.questions[0]))
                .questions[0].isVisible,
